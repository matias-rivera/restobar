...
const DashboardScreen = ({history}) => {
    ...
    const {loading:loadingSales, error:errorSales, orders: sales} = orderAllSales
    ...
    const getTodayDelivery = (sales) => {
        const salesToday = getTodaySales(sales)
        return salesToday.reduce((acc, item) => acc + (item.delivery ? 1 : 0), 0)
    }
        ...
        var indents = [];
            ...
            indents.push(
                <tr key={sales[i].id}>
                    ...
                    <td>
                        <h4>
                            <span className={'badge bg-success'}>${sales[i].total}</span> 
                        </h4>
                    </td>
                    <td>
                        <h4>
                            <span className={'badge bg-warning'}>{sales[i].total_products}</span> 
                        </h4>
                    </td>
                    <td><Link to={`/order/${sales[i].id}/view`} className='btn btn-info'><i class="fas fa-search"></i></Link></td>
                </tr>
            );
            }
        return indents;
